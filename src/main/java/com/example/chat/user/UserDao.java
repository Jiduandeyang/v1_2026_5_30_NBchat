...
public class UserDao {
    public User findById(Connection connection, long id) throws SQLException {
        return Jdbc.one(connection,
                "SELECT id,username,qq_email,nickname,avatar_url,background_url,signature,role,disabled FROM users WHERE id=?",
    ...
    public List<User> search(Connection connection, String q) throws SQLException {
        ...
        return Jdbc.list(connection,
                "SELECT id,username,qq_email,nickname,avatar_url,background_url,signature,role,disabled FROM users WHERE disabled=0 AND (username LIKE ? OR qq_email LIKE ? OR nickname LIKE ?) LIMIT 30",
                ps -> {
